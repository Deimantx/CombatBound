...
export function InventoryDetails({ entry, game }: { entry?: InventoryViewEntry; game: ReturnType<typeof useGameStore.getState>["game"] }) {
  ...
  const tooltip = resolved ? buildPlayerItemInstanceTooltip(resolved, { equipped: entry.equipped, equippedSlot: entry.equippedSlot, masteryLevel }) : buildItemTooltip(entry.definition, { quantity: entry.quantity, masteryLevel });
  ...
  return <aside className="inventory-details-pane" data-ui-panel="inventoryDetails" data-debug-kind="inventory-details-pane">
    ...
    {resolved ? <>
      <div className="detail-summary-grid">{presentation.modified ? <><span className="detail-badge">{resolved.instance.quality > 0 ? `Quality ${resolved.instance.quality}%` : ""}</span>{resolved.instance.quality <= 0 && <span className="detail-badge">No quality</span>}<span className="detail-badge">{resolved.instance.upgradeLevel > 0 ? `Upgrade +${resolved.instance.upgradeLevel}` : "No upgrade"}</span><span className="detail-badge">{resolved.instance.affixes.length} modifier{resolved.instance.affixes.length === 1 ? "" : "s"}</span></> : <span className="detail-badge">Unmodified</span>}</div>
