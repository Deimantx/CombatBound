...
export function InventoryDetails({ entry, game }: { entry?: InventoryViewEntry; game: ReturnType<typeof useGameStore.getState>["game"] }) {
  ...
  return <aside className="inventory-details-pane" data-ui-panel="inventoryDetails" data-debug-kind="inventory-details-pane">
    ...
    <div className="detail-badge-row">{entry.equipped && <span className="detail-badge is-equipped">Equipped{currentSlot ? ` - ${getEquipmentSlotDefinition(currentSlot).label}` : ""}</span>}{presentation.materialTier && <span className="detail-badge">{presentation.materialTier} Tier</span>}{presentation.weaponFamily && presentation.weaponArchetype && <span className="detail-badge">{presentation.weaponFamily} - {presentation.weaponArchetype}</span>}{presentation.upgradeProgress && <span className="detail-badge">{presentation.specialization?.label ?? "Unspecialized"} - {presentation.upgradeProgress.unlocked} / {presentation.upgradeProgress.total} Upgrades</span>}</div>
    <p className="detail-description">{entry.definition.description}</p>
    {container && !resolved && <DetailSection title="Loot container"><div className="detail-action-row"><span className="detail-muted">{container.description}</span><button type="button" className="button button-primary" onClick={() => openLootContainer(entry.definition.id)} disabled={entry.quantity <= 0}>Open</button></div></DetailSection>}
    {resolved ? <>
      <DetailSection title="Upgrade contributions">{presentation.modifiers.length ? <div className="detail-modifier-list">{presentation.modifiers.map((modifier) => <div key={modifier.id}><span>{modifier.label}</span><strong>{modifier.value}</strong></div>)}</div> : <span className="detail-muted">No upgrade nodes unlocked.</span>}</DetailSection>
      ...
      {baseOpen && <div className="detail-stat-list">{baseStatRows.map((stat) => <div key={stat.key}><span>{stat.label}</span><strong>{stat.value}</strong></div>)}</div>}
      {slotTargets.length > 0 && <><>{replacement && <InventoryReplacementContext presentation={replacement} />}</><DetailSection title="Equip"><div className="detail-action-row">{slotTargets.length === 1 ? <span className="detail-badge">Target: {slotTargets[0].label}{currentSlot === slotTargets[0].id ? " (equipped)" : ""}</span> : <div className="inventory-equip-targets" aria-label="Choose equipment target">{slotTargets.map((slot) => <button type="button" key={slot.id} className={chosenSlot === slot.id ? "is-selected" : ""} onClick={() => setTargetSlot(slot.id)}>{slot.label}{currentSlot === slot.id ? " (equipped)" : ""}</button>)}</div>}<button type="button" className="button button-primary" disabled={!validation?.valid || combatLocked || isCurrent} onClick={equip}>{actionLabel}</button></div>{combatLocked && <p className="inventory-lock-note">Stop combat to change equipment.</p>}{validation?.reason === "hunter-rank" && <p className="inventory-lock-note">Requires Hunter Rank {entry.definition.requiredHunterRank}. Current Hunter Rank {hunterRank}.</p>}{validation?.reason === "proficiency-level" && <p className="inventory-lock-note">Requires {proficiencyById[validation.proficiencyId ?? ""]?.name ?? validation.proficiencyId ?? "weapon proficiency"} Level {validation.requiredLevel}.</p>}</DetailSection>{!isCurrent && <InventoryBuildChanges rows={equipmentPreview?.comparison ?? []} replacementName={replacement?.current?.name} slotLabel={chosenSlot ? getEquipmentSlotDefinition(chosenSlot).label : undefined} />}</>}
