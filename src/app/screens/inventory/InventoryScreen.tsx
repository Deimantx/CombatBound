...
export function InventoryScreen() {
  ...
  return <div className="screen inventory-screen" data-debug-screen="inventory">
    ...
    <Panel title="Inventory toolbar" subtitle="Real gameplay items and combat rewards" icon={Backpack} panelId="inventoryToolbar" screen="inventory" className="inventory-toolbar">
      <div className="inventory-toolbar-row"><SearchField value={query} onChange={setQuery} placeholder="Search items" /><div className="toolbar-actions"><button className="button button-ghost button-small" onClick={() => setSort(sort === 'name' ? 'quantity' : 'name')}><ArrowDownUp size={14} />Sort: {sort === 'name' ? 'Name' : 'Quantity'}</button><button className="button button-ghost button-small"><Filter size={14} />Filters</button></div></div>
      <SegmentedTabs items={categories} active={filter as typeof categories[number]} onChange={setFilter} label="Inventory categories" />
    </Panel>
    <div className="inventory-layout">
      <Panel title="Carried items" subtitle={`${entries.length} owned entries · combat loot appears here immediately`} icon={Backpack} panelId="inventoryBank" screen="inventory" className="inventory-bank">
        <div className="inventory-summary"><span>{Object.keys(game.inventory.instances).length} equipment instances</span><span>{Object.values(game.inventory.stackables).filter((quantity) => quantity > 0).length} stack types</span></div>
        <div className="inventory-grid">{entries.map((entry) => {
          const instance = entry.instanceId ? game.inventory.instances[entry.instanceId] : undefined
          return <GameTooltip key={entry.instanceId ?? entry.definition.id} content={entry.instanceId ? buildItemInstanceTooltip(resolveItemInstance(game.inventory, entry.instanceId)!, { equipped: entry.equipped }) : buildItemTooltip(entry.definition, { quantity: entry.quantity })}>
            <button className={`inventory-card rarity-${entry.definition.rarity} ${selected?.ref === entry.ref ? 'is-selected' : ''}`} onClick={() => selectEntry(entry.ref)} data-debug-kind="inventory-item" data-debug-target-id={entry.instanceId ?? entry.definition.id} data-debug-item-id={entry.definition.id} data-debug-instance-id={entry.instanceId} data-debug-label={entry.definition.name}>
              <span className="item-quantity">{entry.instanceId ? '' : entry.quantity}</span><PlaceholderArt icon={entry.definition.icon} size="medium" variant={entry.definition.rarity === 'rare' ? 'gold' : entry.definition.rarity === 'uncommon' ? 'blue' : 'muted'} /><strong>{entry.definition.name}</strong><small>{entry.definition.category} · {entry.equipped ? 'Equipped' : entry.definition.rarity}</small>
              {instance && <span className="item-modifier-badges" data-debug-kind="item-modifier-badges" data-debug-instance-id={instance.id}>{instance.quality > 0 && <em>Q{instance.quality}</em>}{instance.upgradeLevel > 0 && <em>+{instance.upgradeLevel}</em>}{instance.affixes.length > 0 && <em>{instance.affixes.length} Mods</em>}</span>}
            </button>
          </GameTooltip>
        })}</div>
      </Panel>
      <Panel title="Item details" subtitle="Selected item inspection" icon={SlidersHorizontal} panelId="inventoryDetails" screen="inventory" className="inventory-details">
        {selected && selectedTooltip ? <><GameTooltip content={selectedTooltip}><div className="detail-item-head" data-debug-kind="tooltip-trigger" data-debug-item-id={selected.definition.id} data-debug-instance-id={selected.instanceId}><PlaceholderArt icon={selected.definition.icon} label={selected.definition.name} size="large" variant={selected.definition.rarity === 'rare' ? 'gold' : selected.definition.rarity === 'uncommon' ? 'blue' : 'muted'} /><div><span className="tiny-label">{selected.definition.category.toUpperCase()}</span><h3>{selected.definition.name}</h3><p>{selected.definition.rarity} · {selected.instanceId ? (selected.equipped ? 'Equipped instance' : 'Owned instance') : `${selected.quantity} owned`}</p></div></div></GameTooltip><p className="detail-description">{selected.definition.description}</p>{selectedStats && <div className="detail-stats">{formatItemStats(selectedStats).map((stat) => <StatLine key={stat.label} label={stat.label} value={stat.value} accent="gold" />)}</div>}<div className="detail-footer"><span>{selected.instanceId ? 'Instance' : 'Quantity'}</span><strong>{selected.instanceId ? '1' : selected.quantity}</strong></div></> : <p className="detail-description">No owned item selected.</p>}
      </Panel>
    </div>
  </div>
}
