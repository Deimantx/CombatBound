...
export function InventoryScreen() {
  ...
  return <div className="screen inventory-screen" data-debug-screen="inventory">
    ...
    <div className="inventory-layout"><Panel title="Carried items" subtitle={`${visibleItems.length} item types · combat loot appears here immediately`} icon={Backpack} panelId="inventoryBank" screen="inventory" className="inventory-bank"><div className="capacity-row"><span>Item types</span><strong>{visibleItems.length} <small>/ {itemDefinitions.length}</small></strong></div><div className="capacity-track"><span style={{ width: `${Math.min(100, (visibleItems.length / itemDefinitions.length) * 100)}%` }} /></div><div className="inventory-grid">{visibleItems.map((item) => <GameTooltip key={item.id} content={buildItemTooltip(item, { quantity: game.inventory.quantities[item.id] ?? 0 })}><button className={`inventory-card rarity-${item.rarity} ${selected?.id === item.id ? 'is-selected' : ''}`} onClick={() => selectItem(item.id)} data-debug-kind="inventory-item" data-debug-target-id={item.id} data-debug-item-id={item.id} data-debug-label={item.name}><span className="item-quantity">{game.inventory.quantities[item.id]}</span><PlaceholderArt icon={item.icon} size="medium" variant={item.rarity === 'rare' ? 'gold' : item.rarity === 'uncommon' ? 'blue' : 'muted'} /><strong>{item.name}</strong><small>{item.category} · {item.rarity}</small></button></GameTooltip>)}</div></Panel>
      <Panel title="Item details" subtitle="Selected item inspection" icon={SlidersHorizontal} panelId="inventoryDetails" screen="inventory" className="inventory-details"><GameTooltip content={buildItemTooltip(selected, { quantity: selectedQuantity })}><div className="detail-item-head" data-debug-kind="tooltip-trigger" data-debug-item-id={selected.id}><PlaceholderArt icon={selected.icon} label={selected.name} size="large" variant={selected.rarity === 'rare' ? 'gold' : selected.rarity === 'uncommon' ? 'blue' : 'muted'} /><div><span className="tiny-label">{selected.category.toUpperCase()}</span><h3>{selected.name}</h3><p>{selected.rarity} · {selectedQuantity} owned</p></div></div></GameTooltip><p className="detail-description">{selected.description}</p>{selected.stats && <div className="detail-stats">{Object.entries(selected.stats).map(([key, value]) => <StatLine key={key} label={formatItemStats({ [key]: value } as NonNullable<typeof selected.stats>)[0]?.label ?? key} value={formatItemStats({ [key]: value } as NonNullable<typeof selected.stats>)[0]?.value ?? `${value}`} accent="gold" />)}</div>}<div className="detail-footer"><span>Quantity</span><strong>{selectedQuantity}</strong></div></Panel></div>
  </div>
}
