...
import { getAreasForRegion, getLocationsForSubArea, getRegionsForContinent, getSubAreasForArea, isCombatLocationAvailable, locationBreadcrumb, locationParentBreadcrumb } from '../../../game/world/worldSelectors'
...
type WorldTier = 'continent' | 'region' | 'area' | 'sub-area'
type WorldNode = { id: string; name: string; description: string; availability: 'available' | 'locked' | 'coming-soon'; requiredCombatLevel?: number; presentation: { iconKey: keyof typeof worldIcons } }
...
export function CombatWorldBrowser() {
  ...
  const activeBreadcrumb = activeLocation ? locationParentBreadcrumb(activeLocation.id) : undefined
  ...
  return <Panel title="Combat world" subtitle={open ? 'Choose a territory, then hunt its changing population.' : activeLocation ? `Current Hunt · ${activeLocation.name}` : `Selected · ${location?.name ?? 'No location'}`} icon={Map} panelId="combatWorldBrowser" screen="combat" className="combat-world-browser" actions={<button className="button button-ghost button-small" onClick={() => setOpen((value) => !value)}>{open ? 'Collapse' : 'Expand'}</button>}>
    {open ? <>
      {activeLocation && <HuntContext activeLocation={activeLocation} browsingLocation={activeLocation.id === location?.id ? undefined : location} activeBreadcrumb={activeBreadcrumb} browsingBreadcrumb={location ? locationParentBreadcrumb(location.id) : undefined} />}
      <div className="world-hierarchy-stack">
        ...
        <WorldTierRow tier="sub-area" label="SUB-AREA" nodes={subAreas} selectedId={subArea?.id} onSelect={selectSubArea} totalLevel={totalLevel} />
      </div>
      <div className="world-destination-layout">
...
function HuntContext({ activeLocation, browsingLocation, activeBreadcrumb, browsingBreadcrumb }: { activeLocation: CombatLocationDefinition; browsingLocation?: CombatLocationDefinition; activeBreadcrumb?: string; browsingBreadcrumb?: string }) {
  return <div className={`world-hunt-context ${browsingLocation ? 'has-browsing' : ''}`} data-debug-kind="hunt-context" data-debug-label="Current hunt context"><div><span className="tiny-label">CURRENT HUNT</span><strong>{activeLocation.name}</strong><small>{activeBreadcrumb}</small></div>{browsingLocation && <div><span className="tiny-label">BROWSING</span><strong>{browsingLocation.name}</strong><small>{browsingBreadcrumb}</small></div>}</div>
}
...
function LocationPreview({ location, active, available, activeLocationId, onStart }: { location?: CombatLocationDefinition; active: boolean; available: boolean; activeLocationId: string | null; onStart?: () => void }) {
  if (!location) return <div className="world-location-preview empty-state" data-debug-kind="combat-location-preview" data-debug-label="No combat location"><Target size={24} /><strong>No combat location</strong><p>Select a sub-area with a hunting destination.</p></div>
  const family = enemyFamilyById[location.familyId]?.name ?? location.familyId
