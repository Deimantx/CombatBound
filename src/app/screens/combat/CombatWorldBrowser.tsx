...
import { combatLocationPresentation, isCombatTargetUnlocked } from './combatLocationPresentation'
...
function LocationPreview({ location, activeLocation, activeTargetId, combatActive, available, hunterRank, selectedTargetId, onSelectTarget, onStart }: { location?: CombatLocationDefinition; activeLocation?: CombatLocationDefinition; activeTargetId: string | null; combatActive: boolean; available: boolean; hunterRank: number; selectedTargetId: string; onSelectTarget: (enemyId: string) => void; onStart: () => void }) {
  ...
  return <section className="combat-location-preview" data-debug-kind="combat-location-preview" data-debug-location-id={location.id} data-debug-label={`Preview ${location.name}`}>
    ...
    <div className="location-rank-requirement"><span>RANK REQUIRED</span><strong className={available ? 'is-rank-met' : 'is-rank-locked'}>{presentation.requiredHunterRankLabel}</strong></div>
    <div className="location-pool" data-debug-kind="combat-target-list"><span className="tiny-label">SELECT TARGET</span><div className="location-target-list">{presentation.enemies.map((enemy) => { const locked = !isCombatTargetUnlocked(location, enemy.enemyId, hunterRank, available); const selected = selectedId === enemy.enemyId; const fighting = combatActive && activeLocation?.id === location.id && activeTargetId === enemy.enemyId; const individualDropCount = enemy.enemy.loot.length; return <GameTooltip key={enemy.enemyId} content={enemyTooltipModel(enemy.enemyId)}><button type="button" className={`location-target-card ${selected ? 'is-selected' : ''} ${fighting ? 'is-fighting' : ''} ${locked ? 'is-locked' : ''}`} onClick={() => onSelectTarget(enemy.enemyId)} disabled={locked} aria-pressed={selected} aria-label={`${enemy.name} - ${fighting ? 'currently fighting' : locked ? 'locked until the arena is unlocked' : selected ? 'selected for preview' : 'available'}`} data-debug-kind="combat-target-preview" data-debug-enemy-id={enemy.enemyId}><span className="location-target-icon"><EnemyPreviewIcon enemy={enemy.enemy} />{locked && <span className="location-target-lock"><span className="sr-only">Locked</span><Lock size={8} /></span>}</span><span className="location-target-copy"><strong>{enemy.name}</strong><small>{enemy.enemy.enemyTier.toUpperCase()} - {fighting ? 'FIGHTING' : locked ? 'LOCKED' : 'AVAILABLE'}</small>{individualDropCount > 0 && <small className="location-target-drops">{individualDropCount} TARGET DROP{individualDropCount === 1 ? '' : 'S'}</small>}</span></button></GameTooltip> })}</div></div>
    <TargetPreviewInspector location={location} selectedTargetId={selectedId} activeLocationId={activeLocation?.id} activeTargetId={activeTargetId} combatActive={combatActive} />
    <div className="location-preview-footer"><div className="location-preview-action"><button aria-label={!available ? 'Selected arena is locked' : active ? 'Fighting selected target' : activeLocation ? 'Switch to selected target' : 'Fight selected target'} className="button button-primary" onClick={onStart} disabled={!available || active}>{buttonLabel}<Play size={14} /></button></div></div>
  </section>
}
