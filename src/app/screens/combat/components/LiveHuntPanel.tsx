...
export function LiveHuntPanel({ game, stats, location, selectedEnemy, selectedDefinition, onSelectTarget, onCastSpell, onUsePotion, onStartHunt, onStopHunt }: LiveHuntPanelProps) {
  ...
  return <Panel title="Live hunt" subtitle={active ? `${location?.name ?? 'Combat Location'} · Group ${combat.groupNumber} · ${alive} enemies alive` : combat.stopReason ? `Stopped: ${combat.stopReason}` : 'Start a Combat Location Hunt to generate a group'} icon={Swords} panelId="liveCombat" screen="combat" className="live-combat-panel">
    ...
    <div className="combat-resource-hud" data-debug-kind="combat-resource-hud">
      <Resource label="HP" value={combat.playerHp} max={stats.maxHealth} icon={<Heart size={13} />} variant="health" className="resource-health" resource="currentHealth" />
      <Resource label="Stamina" value={combat.stamina} max={combat.maxStamina} icon={<Zap size={13} />} variant="resource" resource="stamina" net={netStamina} detail={`+${stats.staminaRegen.toFixed(1)} regen · -${techniqueStaminaDrain(combat).toFixed(1)} techniques`} />
      <Resource label="Mana" value={combat.mana} max={combat.maxMana} icon={<Sparkles size={13} />} variant="experience" resource="mana" detail={`+${stats.manaRegen.toFixed(1)} regen`} />
    </div>
