...
import { techniqueDrain, combatProgress, getSpellUiState } from './combatUi'
...
export function LiveHuntPanel({ game, stats, location, selectedEnemy, selectedDefinition, onSelectTarget, onCastSpell, onUsePotion, onStartHunt, onStopHunt }: LiveHuntPanelProps) {
  ...
  return <Panel title="Live hunt" subtitle={active ? `${location?.name ?? 'Combat Location'} · Group ${combat.groupNumber} · ${alive} enemies alive` : combat.stopReason ? `Stopped: ${combat.stopReason}` : 'Start a Combat Location Hunt to generate a group'} icon={Swords} panelId="liveCombat" screen="combat" className="live-combat-panel">
    ...
    <div className="combat-resource-hud" data-debug-kind="combat-resource-hud">
      ...
      <Resource label="Energy" value={combat.energy} max={combat.maxEnergy} icon={<Zap size={13} />} variant="resource" resource="energy" detail={`${stats.energyRegen.toFixed(1)} regen · -${techniqueDrain(combat).toFixed(1)} techniques · NET ${netEnergy >= 0 ? '+' : ''}${netEnergy.toFixed(1)}/s`} />
    ...
    <div className="player-attack-progress" data-debug-kind="player-attack"><div><span><Swords size={11} /> YOUR ATTACK</span><strong>{combat.phase === 'active' ? `${Math.max(0, combat.playerAttackTimer).toFixed(1)}s` : combat.phase === 'recovery' ? 'Paused during recovery' : 'Waiting for target'}</strong></div><ProgressBar value={combat.phase === 'active' ? combatProgress(combat.playerAttackTimer, combat.playerAttackInterval) : 0} variant="attack" /><small>{selectedEnemy && !selectedEnemy.defeated ? `Target: ${selectedEnemy.displayName}` : 'Select an enemy target'}</small></div>
    ...
    <div className="spell-controls"><div className="section-title"><span className="tiny-label">COMBAT ACTIONS</span><small>{selectedAction?.interruptible ? 'INTERRUPT AVAILABLE' : 'Spells and consumables'}</small></div><div className="spell-grid">{spellDefinitions.map((spell) => { const runtime = combat.spells.find((entry) => entry.spellId === spell.id); const state = getSpellUiState(spell, runtime, combat, selectedAction); return <button key={spell.id} className={`spell-button is-${state.tone}`} onClick={() => onCastSpell(spell.id)} disabled={!state.enabled} title={`${spell.description} · ${spell.cooldownSeconds}s cooldown`} data-debug-kind="spell" data-debug-spell-id={spell.id}><PlaceholderArt icon={spell.icon} size="small" variant={state.enabled ? 'gold' : 'muted'} /><span><strong>{spell.name}</strong><small>{spell.cost} ADR · {state.status}</small></span></button> })}<button className={`spell-button potion-button ${combat.potionCooldownRemaining > 0 ? 'is-cooldown' : ''}`} onClick={onUsePotion} disabled={combat.phase !== 'active' || combat.potionCooldownRemaining > 0 || (game.inventory.quantities['item.healing-potion'] ?? 0) <= 0 || combat.playerHp >= stats.maxHealth} title="Restore health with a Healing Potion" data-debug-kind="potion"><PlaceholderArt icon="heart" size="small" variant="gold" /><span><strong>Healing Potion ×{game.inventory.quantities['item.healing-potion'] ?? 0}</strong><small>{combat.potionCooldownRemaining > 0 ? `COOLDOWN ${combat.potionCooldownRemaining.toFixed(1)}s` : 'CONSUMABLE'}</small></span></button></div></div>
...
function EnemyCard({ enemy, selected, onSelect }: { enemy: EnemyCombatInstance; selected: boolean; onSelect: (instanceId: string) => void }) {
  ...
  return <button className={`enemy-combat-card ${selected ? 'is-targeted' : ''} ${enemy.defeated ? 'is-defeated' : ''} ${action ? `has-special danger-${action.danger}` : ''}`} onClick={() => onSelect(enemy.instanceId)} aria-label={label} aria-pressed={selected} data-debug-kind="combat-enemy" data-debug-enemy-id={enemy.enemyId} data-debug-instance-id={enemy.instanceId} data-debug-label={enemy.displayName}><div className="enemy-card-top"><PlaceholderArt icon={definition.icon} size="small" variant={definition.accent} /><span><strong>{selected && <Crosshair size={11} />} {enemy.displayName}</strong><small>{enemy.defeated ? 'DEFEATED' : `${Math.floor(enemy.currentHealth)} / ${enemy.maxHealth} HP`}</small></span><em>{selected ? 'TARGETED' : enemy.defeated ? 'DEFEATED' : 'SELECT'}</em></div>{!enemy.defeated && <><ProgressBar value={(enemy.currentHealth / enemy.maxHealth) * 100} variant="health" /><div className="enemy-card-timer"><span><Timer size={11} /> NORMAL ATTACK</span><strong>{Math.max(0, enemy.attackTimer).toFixed(1)}s</strong></div>{!action && <ProgressBar value={combatProgress(enemy.attackTimer, enemy.attackInterval)} variant="attack" />}{action && <div className={`special-intent danger-${action.danger}`}><div><span>⚠ {action.name}</span><strong>{Math.max(0, enemy.currentAction!.remainingSeconds).toFixed(1)}s</strong></div><ProgressBar value={combatProgress(enemy.currentAction!.remainingSeconds, enemy.currentAction!.totalSeconds)} variant="attack" /><small>{action.danger.toUpperCase()} DANGER · {action.interruptible ? 'INTERRUPTIBLE' : 'UNINTERRUPTIBLE'}</small></div>}</>}</button>
}

function Resource({ label, value, max, icon, variant, detail, className = '', resource }: { label: string; value: number; max: number; icon: React.ReactNode; variant: 'health' | 'resource' | 'experience'; detail?: string; className?: string; resource: string }) {
  const rateClass = detail?.includes('NET -') ? 'is-negative' : detail?.includes('NET +') ? 'is-positive' : ''
  return <div className={`resource-block ${className}`} data-debug-kind="combat-resource" data-debug-resource={resource}><div><span>{icon} {label}</span><strong>{Math.floor(value)} / {max}</strong></div><ProgressBar value={(value / max) * 100} variant={variant} ariaLabel={`${label} ${Math.floor(value)} of ${max}`} />{detail && <small className={`resource-rate ${rateClass}`}>{detail}</small>}</div>
}
