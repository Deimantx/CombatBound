...
import { techniqueStaminaDrain, combatProgress, combatTimerLabel, formatPercent, getSpellUiState, useSmoothCombatProgress } from './combatUi'
...
export function LiveHuntPanel({ game, stats, location, selectedEnemy, selectedDefinition, onSelectTarget, onCastSpell, onUsePotion, onStartHunt, onStopHunt }: LiveHuntPanelProps) {
  ...
  const selectedMatchup = getSelectedTargetMatchup(combat, stats, game.progression, selectedEnemy)
...
function Resource({ label, value, max, shield = 0, icon, variant, net, className = '', resource }: { label: string; value: number; max: number; shield?: number; icon: ReactNode; variant: 'health' | 'resource' | 'experience'; net?: number; className?: string; resource: 'currentHealth' | 'stamina' | 'mana' }) {
  ...
  return <GameTooltip content={{ ...buildStatTooltip(resource, value), rows: [{ label: 'Current', value: displayValue, tone: resource === 'currentHealth' ? 'red' : 'blue' }] }}><div className={`resource-block resource-${resource === 'currentHealth' ? 'health' : resource} ${className}`} data-debug-kind="combat-resource" data-debug-resource={resource} data-debug-stat-key={resource}><div className="resource-heading"><span>{icon} {label}</span><strong>{valueLabel}</strong></div>{bar}</div></GameTooltip>
}
