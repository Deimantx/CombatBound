...
import { buildSpellTooltip, buildStatTooltip } from '../../../../game/presentation/tooltipBuilders'
import { calculateEffectiveSpell } from '../../../../game/progression/spellProgression'
import type { EnemyActionDefinition, EnemyCombatInstance, CombatState } from '../../../../game/combat/combatTypes'
...
import { techniqueStaminaDrain, combatProgress, combatTimerLabel, getSpellUiState, useSmoothCombatProgress } from './combatUi'
...
export function LiveHuntPanel({ game, stats, location, selectedEnemy, selectedDefinition, onSelectTarget, onCastSpell, onUsePotion, onStartHunt, onStopHunt }: LiveHuntPanelProps) {
  ...
  return <Panel title="Live hunt" subtitle={active ? `${location?.name ?? 'Combat Location'} · Group ${combat.groupNumber} · ${alive} enemies alive` : combat.stopReason ? `Stopped: ${combat.stopReason}` : 'Start a Combat Location Hunt to generate a group'} icon={Swords} panelId="liveCombat" screen="combat" className="live-combat-panel">
    ...
    <div className="combat-resource-hud" data-debug-kind="combat-resource-hud"><Resource label="HP" value={combat.playerHp} max={stats.maxHealth} icon={<Heart size={13} />} variant="health" resource="currentHealth" /><Resource label="Stamina" value={combat.stamina} max={combat.maxStamina} icon={<Zap size={13} />} variant="resource" resource="stamina" net={netStamina} detail={`+${stats.staminaRegen.toFixed(1)} regen · -${techniqueStaminaDrain(combat).toFixed(1)} techniques`} /><Resource label="Mana" value={combat.mana} max={combat.maxMana} icon={<Sparkles size={13} />} variant="experience" resource="mana" detail={`+${stats.manaRegen.toFixed(1)} regen`} /></div>
    ...
    <div className="spell-controls"><div className="section-title"><span className="tiny-label">COMBAT ACTIONS</span><small>{selectedAction?.interruptible ? 'INTERRUPT AVAILABLE' : 'Spells and consumables'}</small></div>{selectedAction?.interruptible && <div className="interrupt-window"><Crosshair size={13} /><strong>INTERRUPT WINDOW OPEN</strong><span>Disrupting Pulse can stop {selectedAction.name}.</span></div>}<div className="spell-grid">{spellDefinitions.map((spell) => { const runtime = combat.spells.find((entry) => entry.spellId === spell.id); const effectiveSpell = calculateEffectiveSpell(spell, game.progression, selectedEnemy ? { targetHpFraction: selectedEnemy.currentHealth / selectedEnemy.maxHealth, targetEffects: selectedEnemy.effects } : undefined); const state = getSpellUiState(spell, runtime, combat, selectedAction, effectiveSpell); const button = <button className={`spell-button is-${state.tone} ${spell.id === 'spell.disrupting-pulse' && state.enabled ? 'is-interrupt-ready' : ''}`} onClick={() => onCastSpell(spell.id)} disabled={!state.enabled} data-debug-kind="spell" data-debug-spell-id={spell.id} data-debug-label={spell.name}><PlaceholderArt icon={spell.icon} size="small" variant={state.enabled ? 'gold' : 'muted'} /><span><strong>{spell.name}</strong><small>{effectiveSpell.manaCost} MANA · {state.status}</small></span></button>; return state.enabled ? <GameTooltip key={spell.id} content={buildSpellTooltip(spell, game.progression)}>{button}</GameTooltip> : <GameTooltip key={spell.id} content={buildSpellTooltip(spell, game.progression)}><span className="spell-tooltip-host">{button}</span></GameTooltip> })}<GameTooltip content={{ id: 'item.healing-potion', icon: 'heart', title: 'Healing Potion', subtitle: 'Consumable', description: 'Restores health during combat.', rows: [{ label: 'Status', value: potionStatus, tone: potionReady ? 'green' : 'default' }] }}><span className="spell-tooltip-host"><button className={`spell-button potion-button ${potionReady ? 'is-ready' : 'is-invalid'}`} onClick={onUsePotion} disabled={!potionReady} data-debug-kind="potion" data-debug-item-id="item.healing-potion"><PlaceholderArt icon="heart" size="small" variant="gold" /><span><strong>Healing Potion ×{potionQuantity}</strong><small>{potionStatus}</small></span></button></span></GameTooltip></div></div>
...
function EnemyCard({ enemy, selected, onSelect }: { enemy: EnemyCombatInstance; selected: boolean; onSelect: (instanceId: string) => void }) {
  ...
  return <button className={`enemy-combat-card ${selected ? 'is-targeted' : ''} ${enemy.defeated ? 'is-defeated' : ''} ${action ? `has-special danger-${action.danger}` : ''}`} onClick={() => onSelect(enemy.instanceId)} aria-label={label} aria-pressed={selected} data-debug-kind="combat-enemy" data-debug-enemy-id={enemy.enemyId} data-debug-instance-id={enemy.instanceId} data-debug-label={enemy.displayName}><div className="enemy-card-top"><PlaceholderArt icon={definition.icon} size="small" variant={definition.accent} /><span><strong>{selected && <Crosshair size={11} />} {enemy.displayName}</strong><small>{enemy.defeated ? 'DEFEATED' : `${Math.floor(enemy.currentHealth)} / ${enemy.maxHealth} HP`}</small></span><em className={selected ? 'target-tag' : ''}>{selected ? 'TARGETED' : enemy.defeated ? 'DEFEATED' : 'SELECT'}</em></div>{!enemy.defeated && <><ProgressBar value={(enemy.currentHealth / enemy.maxHealth) * 100} variant="health" className="enemy-health-bar" ariaLabel={`${enemy.displayName} health`} /><div className="enemy-card-timer"><span><Timer size={11} /> NORMAL ATTACK</span><strong>{combatTimerLabel(enemy.attackTimer, enemy.attackInterval)}</strong></div>{!action && <ProgressBar value={normalAttackProgress.value} variant="attack" className={`enemy-action-progress ${normalAttackProgress.isResetting ? 'is-attack-resetting' : ''}`} ariaLabel={`${enemy.displayName} normal attack progress`} />}{action && <div className={`special-intent danger-${action.danger}`}><div className="special-intent-heading"><span>⚠ {action.name}</span><strong>{combatTimerLabel(enemy.currentAction!.remainingSeconds, enemy.currentAction!.totalSeconds)}</strong></div><ProgressBar value={combatProgress(enemy.currentAction!.remainingSeconds, enemy.currentAction!.totalSeconds)} variant="attack" className="enemy-action-progress" ariaLabel={`${action.name} progress`} /><small><span>{action.danger.toUpperCase()} DANGER</span><strong>{action.interruptible ? 'INTERRUPTIBLE' : 'UNINTERRUPTIBLE'}</strong></small></div>}</>}</button>
}

function Resource({ label, value, max, icon, variant, detail, net, className = '', resource }: { label: string; value: number; max: number; icon: ReactNode; variant: 'health' | 'resource' | 'experience'; detail?: string; net?: number; className?: string; resource: 'currentHealth' | 'stamina' | 'mana' }) {
  const rateClass = net !== undefined && net < 0 ? 'is-negative' : net !== undefined && net > 0 ? 'is-positive' : ''
  return <GameTooltip content={{ ...buildStatTooltip(resource, value), rows: [{ label: 'Current', value: `${Math.floor(value)} / ${Math.floor(max)}`, tone: resource === 'currentHealth' ? 'red' : 'blue' }] }}><div className={`resource-block ${className}`} data-debug-kind="combat-resource" data-debug-resource={resource} data-debug-stat-key={resource}><div className="resource-heading"><span>{icon} {label}</span><strong>{Math.floor(value)} / {max}</strong></div><ProgressBar value={(value / max) * 100} variant={variant} className="resource-bar" ariaLabel={`${label} ${Math.floor(value)} of ${max}`} />{net !== undefined && <strong className={`resource-net ${rateClass}`}>Net Stamina: {net >= 0 ? '+' : ''}{net.toFixed(1)}/s</strong>}{detail && <small className="resource-rate-detail">{detail}</small>}</div></GameTooltip>
}
