...
function Resource({ label, value, max, shield = 0, icon, variant, detail, net, className = '', resource }: { label: string; value: number; max: number; shield?: number; icon: ReactNode; variant: 'health' | 'resource' | 'experience'; detail?: string; net?: number; className?: string; resource: 'currentHealth' | 'stamina' | 'mana' }) {
  ...
  return <GameTooltip content={{ ...buildStatTooltip(resource, value), rows: [{ label: 'Current', value: displayValue, tone: resource === 'currentHealth' ? 'red' : 'blue' }] }}><div className={`resource-block ${className}`} data-debug-kind="combat-resource" data-debug-resource={resource} data-debug-stat-key={resource}><div className="resource-heading"><span>{icon} {label}</span><strong>{displayValue}</strong></div><ProgressBar value={(value / max) * 100} variant={variant} className="resource-bar" ariaLabel={`${label} ${ariaValue}`} />{net !== undefined && <strong className={`resource-net ${rateClass}`}>Net Stamina: {net >= 0 ? '+' : ''}{net.toFixed(1)}/s</strong>}{detail && <small className="resource-rate-detail">{detail}</small>}</div></GameTooltip>
}
