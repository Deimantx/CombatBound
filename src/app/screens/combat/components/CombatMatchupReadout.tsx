...
export function CombatMatchupReadout({ game, stats, selectedEnemy }: { game: GameState; stats: HunterCombatStats; selectedEnemy?: EnemyCombatInstance }) {
  ...
  return <div className="combat-matchup" data-debug-kind="combat-matchup" data-debug-hit-chance={matchup?.playerHitChance ?? ''} data-debug-enemy-hit-chance={matchup?.enemyHitChance ?? ''}>
    ...
    {matchup ? <>
      ...
      <MatchupMetric label="Hit Chance" value={formatPercent(matchup.playerHitChance)} tone="gold" tooltip={{ id: 'combat.matchup-hit-chance', title: 'Hit Chance', description: 'Your Accuracy is compared against the target Evasion to determine the miss check. Dodge, Parry, and Block are resolved separately.', rows: [{ label: 'Your Accuracy', value: `${Math.round(matchup.playerAccuracy)}`, tone: 'blue' }, { label: 'Target Evasion', value: `${Math.round(matchup.targetEvasion)}`, tone: 'blue' }, { label: 'Result', value: formatPercent(matchup.playerHitChance), tone: 'gold' }] }} />
      <MatchupMetric label="Target Evasion" value={`${Math.round(matchup.targetEvasion)}`} />
      <MatchupMetric label="Dodge Chance" value={formatPercent(matchup.targetDodgeChance)} />
      <MatchupMetric label="Parry Chance" value={formatPercent(matchup.targetParryChance)} />
