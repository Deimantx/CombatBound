      ...
      {selectedEnemy && definition ? <>
        <div className="target-card-top">
          ...
          <div>
            ...
            <p>{definition.family} · Group {combat.groupNumber}</p>
        ...
        {definition.actions.length > 0 && <div className="enemy-actions-detail">
          <span className="tiny-label">SPECIAL ACTIONS</span>
          {definition.actions.map((action) => {
            const current = selectedEnemy.currentAction?.actionId === action.id
            return <div className={`enemy-action-detail ${current ? `is-casting danger-${action.danger}` : ''}`} key={action.id}>
              <strong>{current ? 'CASTING · ' : ''}{action.name}</strong>
              <small>{action.danger.toUpperCase()} DANGER · {action.interruptible ? 'INTERRUPTIBLE' : 'UNINTERRUPTIBLE'}</small>
              {current && <ProgressBar value={combatProgress(selectedEnemy.currentAction!.remainingSeconds, selectedEnemy.currentAction!.totalSeconds)} variant="attack" ariaLabel={`${action.name} progress`} />}
            </div>
          })}
        </div>}
