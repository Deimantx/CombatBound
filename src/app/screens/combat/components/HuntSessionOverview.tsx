...
export function HuntSessionOverview({ game, tab, onTabChange }: { game: GameState; tab: SessionTab; onTabChange: (tab: SessionTab) => void }) {
  ...
  const sampleValue = (value: string) => rates.rateSampleReady ? value : 'â€”'
  ...
  const summaryMetrics = [
    ...
    metric('Avg Kill', rates.averageKillSeconds === null ? 'â€”' : formatDuration(rates.averageKillSeconds), 'average-kill-seconds', 'Average elapsed Hunt time per defeated enemy.'),
  ]
  const lootMetrics = [
  ...
  const note = !rates.rateSampleReady ? 'Collecting rate data â€” hourly projections appear after 10 seconds.' : combat.stopReason ? `Last stop: ${combat.stopReason}` : tab === 'Loot' ? 'Loot totals are permanent; hourly values are projections from this Hunt.' : tab === 'Progression' ? 'Only Proficiencies that gained XP during this Hunt are shown.' : 'Cumulative metrics include combat and recovery time.'
...
function ProgressionLevelBar({ label, progress, kind }: { label: string; progress: { level: number; progressFraction: number; isMaxLevel: boolean }; kind: string }) {
  ...
  return <div className="progression-level-bar" data-debug-kind="progression-level-bar" data-debug-progress-kind={kind}><div className="progression-level-bar-heading"><span>{label}</span><strong>{progress.isMaxLevel ? 'MAX LEVEL' : `${progress.level} â†’ ${progress.level + 1}`}</strong></div><div className="progression-level-bar-labels"><span>{progress.level}</span><strong>{percent}%</strong><span>{progress.isMaxLevel ? 'MAX' : progress.level + 1}</span></div><ProgressBar value={percent} variant="experience" ariaLabel={`${label} level ${progress.level} progress`} /></div>
}
...
function formatEta(xpToNextLevel: number, xpPerHour: number, rateReady: boolean, isMaxLevel: boolean) {
  ...
  if (!rateReady || xpPerHour <= 0 || xpToNextLevel <= 0) return 'â€”'
