...
import { getProficiencyLevelProgressForState, getProficiencyProgress, getProficiencyXpToNextLevel } from '../../../game/progression/proficiencyProgression'
...
function ProficiencyTile({ definition, selected, active, progression, onSelect }: { definition: typeof proficiencyDefinitions[number]; selected: boolean; active: boolean; progression: Progression; onSelect: (id: CombatProficiencyId) => void }) {
  ...
  const levelProgress = getProficiencyLevelProgressForState(progression, definition.id)
  const percent = levelProgress.progressFraction * 100
  const levelLabel = definition.id === 'magic-arts' ? `Lv ${levelProgress.level} / ${definition.maxLevel}` : levelProgress.level > 0 ? `Lv ${levelProgress.level} / ${definition.maxLevel}` : 'UNTRAINED'
