...
export function CollectionScreen() {
  ...
  const entries = tab === 'Items'
    ? itemDefinitions.map((item) => ({ id: item.id, name: item.name, icon: item.icon, meta: `${item.category} · ${item.rarity}`, discovered: game.collection.discoveredItems.includes(item.id), description: item.description, defeats: 0, sources: combatLocationDefinitions.filter((location) => location.sharedLoot?.some((drop) => drop.itemId === item.id) || location.targets.some((entry) => enemyById[entry.enemyId]?.loot.some((drop) => drop.itemId === item.id))).map((location) => location.name) }))
    : enemyDefinitions.map((enemy) => ({ id: enemy.id, name: enemy.name, icon: enemy.icon, meta: `${enemy.family} · ${game.collection.targets[enemy.id]?.defeats ?? 0} defeats`, discovered: game.collection.targets[enemy.id]?.discovered ?? false, description: `${getEnemyResolvedTraits(enemy).map((trait) => `${trait.definition.name}: ${trait.rank.description}`).join(' ')} Weaknesses: ${Object.entries(enemy.resistances).filter(([, value]) => value < 0).map(([type]) => type).join(', ') || 'none'}.`, defeats: game.collection.targets[enemy.id]?.defeats ?? 0, sources: combatLocationDefinitions.filter((location) => location.targets.some((entry) => entry.enemyId === enemy.id)).map((location) => location.name) }))
