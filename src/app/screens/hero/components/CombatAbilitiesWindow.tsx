...
export function CombatAbilitiesWindow({ game, onOpenAutomation }: { game: GameState; onOpenAutomation: (actionId?: string, createRule?: boolean) => void }) {
  ...
  const filtered = entries.filter((entry) => {
    ...
    if (filter === "magic") return entry.kind === "spell";
    return true;
  }).filter((entry) => !normalized || `${entry.kind} ${entry.name} ${entry.description} ${entry.kind === "spell" ? entry.magicProficiencyId : entry.kind === "active-action" ? entry.proficiencyId : entry.id}`.toLowerCase().includes(normalized));
  ...
  const group = (id: string, label: string, icon: string, groupEntries: CombatAbilityCatalogueEntry[]) => groupEntries.length ? <CatalogueAccordionGroup key={id} id={`all.${id}`} label={label} icon={icon} count={groupEntries.length} expanded={normalized.length > 0 || expanded.has(`all.${id}`)} onToggle={() => setGroup(`all.${id}`)} debugGroupType={id === "magic" ? "spellbook" : "combat-ability"}>{groupEntries.map((entry) => <LibraryEntry key={entryKey(entry)} entry={entry} game={game} selected={entryKey(entry) === selectedId} locked={locked} drag={drag} onSelect={() => setSelectedId(entryKey(entry))} onDragStart={onDragStart} onDragEnd={() => setDrag({ payload: null, target: null, state: "idle" })} />)}</CatalogueAccordionGroup> : null;
  const renderLibrary = () => {
    ...
    const magic = filtered.filter((entry) => entry.kind === "spell");
    if (filter !== "all") return group(filter, filter === "active-defense" ? "ACTIVE DEFENSE" : filter === "weapon-skills" ? "WEAPON SKILLS" : filter === "magic" ? "MAGIC" : "CORE", filter === "magic" ? "spark" : filter === "weapon-skills" ? "sword" : filter === "active-defense" ? "shield" : "cross", filtered);
    return <>{group("core", "CORE", "cross", core)}{group("active-defense", "ACTIVE DEFENSE", "shield", defense)}{group("weapon-skills", "WEAPON SKILLS", "sword", weapons)}{magicSchoolOrder.map((schoolId) => group(`magic.${schoolId}`, getMagicSchoolPresentation(schoolId).fullLabel, getMagicSchoolPresentation(schoolId).icon, magic.filter((entry) => entry.kind === "spell" && entry.magicProficiencyId === schoolId)))}</>;
  };
  return <div className="combat-abilities-window" data-debug-kind="combat-abilities-window">
    <div className="combat-abilities-lock-note">{locked ? "COMBAT ACTIVE · Loadout editing is locked until the Hunt stops." : "Drag any known spell, weapon skill or active defense into the shared five-slot loadout."}</div>
    <div className="combat-abilities-callout"><strong>One Combat Ability loadout.</strong><span>Weapon skills, defenses and known Magic compete for the same five slots. Basic Attack and Consumables remain separate.</span></div>
    ...
    <div className="combat-ability-filters" role="tablist" aria-label="Combat ability filters">{([["all", "ALL"], ["core", "CORE"], ["active-defense", "ACTIVE DEFENSE"], ["weapon-skills", "WEAPON SKILLS"], ["magic", "MAGIC"]] as Array<[LibraryFilter, string]>).map(([value, label]) => <button key={value} className={filter === value ? "is-active" : ""} onClick={() => setFilter(value)}>{label}</button>)}</div>
...
function LibraryEntry({ entry, game, selected, locked, drag, onSelect, onDragStart, onDragEnd }: { entry: CombatAbilityCatalogueEntry; game: GameState; selected: boolean; locked: boolean; drag: DragState; onSelect: () => void; onDragStart: (event: React.DragEvent, payload: DragPayload) => void; onDragEnd: () => void }) {
  ...
  const subtitle = entry.kind === "core" ? "CORE COMBAT · ALWAYS AVAILABLE" : entry.kind === "spell" ? `MAGIC · ${entry.magicProficiencyId ?? "known spell"}` : `${entry.category === "active-defense" ? "ACTIVE DEFENSE" : "WEAPON SKILL"} · ${getActionById(game, actionId, createCombatPreviewContext())?.resourceCost?.stamina ?? 0} Stamina`;
  return <button className={`combat-ability-library-entry ${selected ? "is-selected" : ""} ${dragging ? "is-dragging" : ""}`} onClick={onSelect} draggable={entry.kind !== "core" && !locked} onDragStart={(event) => { if (entry.kind !== "core") onDragStart(event, { actionId }); }} onDragEnd={onDragEnd} data-debug-kind="combat-ability-library-entry" data-debug-ability-id={actionId} data-debug-ability-kind={entry.kind === "active-action" ? entry.category : entry.kind} data-debug-proficiency-id={entry.kind === "active-action" ? entry.proficiencyId : entry.kind === "spell" ? entry.magicProficiencyId : undefined}><PlaceholderArt icon={entry.icon} size="small" variant={entry.kind === "spell" ? "gold" : "blue"} /><span><strong>{entry.name}</strong><small>{subtitle}</small></span><em className={availability && !availability.usable ? "is-invalid" : equippedSlot >= 0 ? "is-equipped" : ""}>{entry.kind === "core" ? "ALWAYS AVAILABLE" : equippedSlot >= 0 ? `EQUIPPED ${equippedSlot + 1}` : availability && !availability.usable ? availability.label : "KNOWN"}</em></button>;
}
