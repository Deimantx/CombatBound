    ...
    <section className="hero-equipment-options hero-equipment-slot-inspector" data-debug-kind="hero-equipment-selected" data-debug-options="true" data-debug-slot-id={slotId} data-debug-expanded="true" data-debug-compatible-count={models.length}>
      <div className="equipment-compatible-workspace" data-debug-kind="equipment-compatible-workspace">
        <div className="equipment-item-list-pane" data-debug-kind="equipment-item-list">
          ...
          <EquipmentCurrentCard slotId={slotId} current={current} defensiveContext={defensiveContext} masteryLevel={masteryLevel} combatLocked={combatLocked} onClearPreview={onClearPreview} onUnequip={onUnequip} />
          <EquipmentCandidateBrowser models={availableModels} slotId={slotId} masteryLevel={masteryLevel} totalCount={models.length} pinned={pinned} hovered={hovered} onSelect={onSelectCandidate} onHover={onHoverCandidate} onLeave={onLeaveCandidate} />
        </div>
        <EquipmentItemComparison slotLabel={definition.label} current={current} candidate={activeCandidate} itemRows={itemRows} buildRows={activeCandidate && !isCurrent ? previewState.comparison : []} combatLocked={combatLocked} masteryLocked={Boolean(activeModel?.masteryLocked)} actionLabel={actionLabel} canEquip={canCommit} onEquip={() => { if (activeCandidate) onEquip(activeCandidate.instance.id); }} />
      </div>
    </section>
  );
}
