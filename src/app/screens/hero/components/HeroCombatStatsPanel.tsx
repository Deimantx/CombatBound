...
import { formatCombatStatValue, labelForStatKey } from "../../../../game/presentation/statFormatting";
...
export function HeroCombatStatsPanel() {
  ...
  const valueFor = (key: string) => {
    if (key === "physicalDirectMitigation") return calculateArmorMitigation(stats.armor);
  ...
  return (
    <aside className={`hero-combat-stats ${preferences.panel ? "is-open" : "is-collapsed"}`} data-debug-kind="hero-combat-stats" data-debug-expanded={preferences.panel ? "true" : "false"}>
      ...
      <div id={panelContentId} className="hero-combat-stats-content" hidden={!preferences.panel}>
        ...
        <div className="hero-stat-groups">
          {combatStatGroups.map((group) => <HeroStatCategory key={group.id} group={group} open={preferences[group.id]} onToggle={() => toggle(group.id)} valueFor={valueFor} />)}
        </div>
        <p className="hero-stats-note">Temporary combat effects are shown on the Combat screen.</p>
      </div>
    </aside>
  );
}

function HeroStatCategory({ group, open, onToggle, valueFor }: { group: (typeof combatStatGroups)[number]; open: boolean; onToggle: () => void; valueFor: (key: string) => number }) {
  ...
  return (
    <section className={`hero-stat-category ${open ? "is-open" : "is-collapsed"}`} data-debug-kind="hero-stat-category" data-debug-category={group.id} data-debug-expanded={open ? "true" : "false"} data-debug-count={group.keys.length}>
      <button type="button" className="hero-stat-category-toggle" onClick={onToggle} aria-expanded={open} aria-controls={contentId}><span><strong>{group.title}</strong><small>{group.keys.length} stats</small></span><DisclosureChevron open={open} size={14} /></button>
      <div id={contentId} className={group.id === "resistances" ? "hero-stat-resistance-grid" : "hero-stat-list"} hidden={!open}>
        {group.keys.map((key) => {
          const value = valueFor(key);
          return <div key={key} className="hero-stat-row" data-debug-kind="hero-stat-row" data-debug-stat={key} data-debug-value={value}><StatLine label={labelForStatKey(key)} value={formatCombatStatValue(key, value)} detail={key === "attackInterval" ? `${(1 / Math.max(0.01, value)).toFixed(2)} attacks/sec` : undefined} accent={key.endsWith("Resistance") ? value > 0 ? "green" : value < 0 ? "red" : undefined : key === "attackPower" ? "gold" : undefined} statKey={key} statValue={value} /></div>;
        })}
      </div>
    </section>
  );
