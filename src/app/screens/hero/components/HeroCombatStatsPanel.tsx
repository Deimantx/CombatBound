    ...
    <section className={`hero-stat-category ${open ? "is-open" : "is-collapsed"}`} data-debug-kind="hero-stat-category" data-debug-category={group.id} data-debug-expanded={open ? "true" : "false"} data-debug-count={group.keys.length}>
      ...
      <div id={contentId} className={group.id === "resistances" ? "hero-stat-resistance-grid" : "hero-stat-list"} hidden={!open}>
        {group.keys.map((key) => {
          ...
          const changed = delta !== undefined && Math.abs(delta) > 0.000001;
          const deltaKind = !changed ? "neutral" : key === "attackInterval" ? delta! < 0 ? "better" : "worse" : delta! > 0 ? "better" : "worse";
...
function StatValue({ statKey, current, preview, delta, deltaKind }: { statKey: string; current: number; preview?: number; delta?: number; deltaKind: "better" | "worse" | "neutral" }) {
  ...
  return <span className="hero-stat-value-comparison"><span>{formatCombatStatValue(statKey, current)}</span>{changed && <><span className="hero-stat-arrow">→</span><strong className="hero-stat-preview-value">{formatCombatStatValue(statKey, preview)}</strong><em className={`hero-stat-delta is-${deltaKind}`}>{formatStatDelta(statKey, delta)}</em></>}</span>;
}

function formatStatDelta(statKey: string, value: number) {
  const sign = value > 0 ? "+" : "";
  if (statKey === "attackInterval") return `${sign}${value.toFixed(2)}s`;
  if (["critChance", "dodgeChance", "parryChance", "blockChance", "blockPower", "statusResistance", "physicalDirectMitigation"].includes(statKey) || statKey.endsWith("Resistance") || statKey === "critDamage") return `${sign}${Math.round(value * 100)}%`;
  if (["staminaRegen", "manaRegen", "healthRegen"].includes(statKey)) return `${sign}${value.toFixed(1)} / sec`;
