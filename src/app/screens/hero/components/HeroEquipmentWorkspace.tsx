...
export function HeroEquipmentWorkspace({ preview, hoveredPreview, previewState, onPreviewChange, onHoverPreview, onEquipCommitted }: {
  ...
  const professions = game.professions;
  ...
  const miningToolLocked = game.mining.active && selected === "tool";
  ...
  const candidateModels = useMemo<EquipmentCandidateModel[]>(() => candidates.map((entry) => {
    const validation = validateEquipmentChange({ instanceId: entry.instance.id, slotId: selected, inventory, equipment, hunterRank, progression, professions });
    const equipped = entry.instance.id === equipment.slots[selected];
    return { entry, validation, equipped, equippedSlot: equippedSlotForInstance(equipment, entry.instance.id), hunterRankLocked: validation.reason === "hunter-rank", proficiencyLocked: validation.reason === "proficiency-level" || validation.reason === "profession-level", canEquip: validation.valid && !combatLocked && !miningToolLocked && !equipped, combatLocked: combatLocked || miningToolLocked };
  }), [candidates, combatLocked, equipment, inventory, hunterRank, professions, progression, selected, miningToolLocked]);
