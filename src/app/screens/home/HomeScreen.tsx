...
export function HomeScreen() {
  ...
  return <div className="screen home-screen" data-debug-screen="home">
    ...
    <div className="home-overview-grid">
      <Panel title="Hunter overview" subtitle="Your current combat profile" icon={Shield} panelId="homeOverview" screen="home" className="home-overview"><div className="overview-primary"><div className="large-avatar"><Shield size={35} /></div><div><h3>Vanguard</h3><p>{activeDefinition?.name ?? "No weapon proficiency"} / {stats.attackDamage} Attack Damage</p><div className="identity-tags"><span>HUNTER RANK {hunterRankProgress.rank}</span><span>POWER {stats.attackDamage}</span></div></div></div><div className="overview-stats"><StatLine label="Current activity" value={activeLocation ? `${activeLocation.name} · ${activeFamily ?? 'Hunt'} · ${game.combat.enemy?.displayName ?? 'Target recovery'}` : 'Idle'} accent="blue" /><StatLine label="Total kills" value={totalDefeats} accent="gold" /><StatLine label="Active proficiency" value={activeDefinition ? `${activeDefinition.name} · Lv ${level}` : 'Untrained'} /></div><div className="home-hp"><div className="home-hp-heading"><span>Current health</span><strong>{playerHealth}</strong></div><ProgressBar value={(game.combat.playerHp / Math.max(1, stats.maxLife ?? 0)) * 100} variant="health" ariaLabel={`Player health ${playerHealth}`} /></div><button className="button button-primary full-button" onClick={() => setScreen('combat')}><Swords size={15} />{game.combat.phase === 'active' ? 'View live combat' : 'Open combat'}</button>{activeLocation && <p className="home-active-breadcrumb">{locationBreadcrumb(activeLocation.id)}</p>}</Panel>
      <Panel title="Hunter Rank" subtitle="Global career progression" icon={Sparkles} panelId="homeHunterRank" screen="home"><div className="hunter-rank-home-grid"><div><span className="tiny-label">HUNTER RANK</span><strong>Rank {hunterRankProgress.rank}</strong></div><div><span className="tiny-label">RANK POINTS</span><strong>{Math.floor(hunterRankProgress.totalPoints).toLocaleString()}</strong></div><div><span className="tiny-label">TO NEXT RANK</span><strong>{hunterRankProgress.isMaxRank ? "MAX" : hunterRankProgress.pointsToNextRank.toLocaleString()}</strong><small>{availablePoints} perk points available</small></div></div><div className="active-proficiency-home"><span className="tiny-label">ACTIVE WEAPON PROFICIENCY</span><strong>{activeDefinition ? `${activeDefinition.name} · Lv ${level}` : 'UNTRAINED'}</strong><small>{active ? `${getProficiencyXpToNextLevel(game.progression, active.proficiencyId).toLocaleString()} XP to next level` : 'Equip a weapon to begin.'}</small>{activeDefinition && <ProgressBar value={Math.max(0, Math.min(100, proficiencyPercent))} variant="experience" ariaLabel="Active proficiency progress" />}</div><button className="button button-ghost full-button" onClick={() => setScreen('proficiencies')}><Award size={15} />Open Proficiencies</button></Panel>
    </div>
    <div className="home-secondary-grid">
      <Panel title="Combat record" subtitle="Permanent gameplay results" icon={Trophy} panelId="homeCombatRecord" screen="home" className="record-panel"><div className="record-grid"><div><strong>{totalDefeats}</strong><span>Total kills</span></div><div><strong>{game.combat.session.highestHit}</strong><span>Highest hit</span></div><div><strong>{game.combat.session.enemiesDefeated}</strong><span>Session defeats</span></div><div><strong>{Math.floor(game.combat.session.damageDealt)}</strong><span>Session damage</span></div></div><div className="record-footer"><span><Award size={14} /> Active proficiency</span><strong>{activeDefinition?.name ?? 'Untrained'}</strong></div></Panel>
