...
import { buildSkillTooltip } from '../../../game/presentation/tooltipBuilders'
  ...
  return <div className="screen home-screen" data-debug-screen="home">
    ...
    <div className="home-overview-grid">
      <Panel title="Hunter overview" subtitle="Your current combat profile" icon={Shield} panelId="homeOverview" screen="home" className="home-overview">
        ...
        <div className="overview-stats"><StatLine label="Current activity" value={activeLocation ? `${activeLocation.name} · ${activeFamily ?? 'Hunt'} · Group ${game.combat.groupNumber}` : 'Idle'} accent="blue" /><StatLine label="Total kills" value={totalDefeats} accent="gold" /><GameTooltip content={buildSkillTooltip(game.progression.trainingFocus)}><div data-debug-kind="tooltip-trigger" data-debug-skill-id={`skill.${game.progression.trainingFocus}`}><StatLine label="Training focus" value={skillById[game.progression.trainingFocus].name} /></div></GameTooltip></div>
      ...
      <Panel title="Combat progression" subtitle="Select one active training focus" icon={Activity} panelId="homeCombatProgression" screen="home">
        <div className="progression-list">{Object.values(game.progression.skills).map((skill) => { const next = xpForLevel(skill.level + 1); const previous = xpForLevel(skill.level); const progress = next === previous ? 100 : ((skill.totalXp - previous) / (next - previous)) * 100; return <GameTooltip key={skill.id} content={buildSkillTooltip(skill.id)}><button className={`progression-row focus-progression ${game.progression.trainingFocus === skill.id ? 'is-focused' : ''}`} onClick={() => setTrainingFocus(skill.id)} data-debug-kind="tooltip-trigger" data-debug-skill-id={`skill.${skill.id}`} data-debug-label={skillById[skill.id].name}><div className="progression-heading"><span>{skillById[skill.id].name}</span><strong>Lv {skill.level}{game.progression.trainingFocus === skill.id ? ' · ACTIVE' : ''}</strong></div><ProgressBar value={Math.max(0, Math.min(100, progress))} variant="experience" showValue ariaLabel={`${skillById[skill.id].name} training progress`} /></button></GameTooltip> })}</div>
