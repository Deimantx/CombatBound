...
export function DebugSpellbookTab({ game, run, debug }: DebugTabProps) {
  ...
  const schools = useMemo(() => magicSchoolOrder.map((schoolId) => ({ schoolId, presentation: getMagicSchoolPresentation(schoolId), spells: spellDefinitions.filter((spell) => spell.magicProficiencyId === schoolId && (filter === "all" || filter === schoolId) && (!normalized || `${spell.id} ${spell.name} ${spell.description} ${getMagicSchoolPresentation(spell.magicProficiencyId).fullLabel} ${spell.damageType ?? ""} ${spell.damage} ${spell.barrierAmount ?? ""} ${(spell.applyEffects ?? []).map(({ effectId }) => spellById[effectId]?.name ?? effectId).join(" ")}`.toLowerCase().includes(normalized))) })).filter((school) => school.spells.length), [filter, normalized]);
...
function DebugSpellRow({ spell, game }: { spell: (typeof spellDefinitions)[number]; game: DebugTabProps["game"] }) {
