...
import { areaById, areaDefinitions } from "../../../../game/data/world/areas";
...
export function DebugEncounterTab({ debug, run }: DebugTabProps) {
  ...
  return <div className="debug-tab-content debug-column"><DebugSection title="Encounter Builder" subtitle="Build a temporary mixed group from the canonical Continent → Region → Area → Combat Location → Enemy hierarchy."><div className="debug-encounter-toolbar"><label>LOCATION <select value={locationId} onChange={(event) => setLocationId(event.target.value)}>{buildLocationOptions()}</select></label><input className="debug-search-input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search enemies, zones or family..." aria-label="Search encounter enemies" /><DebugButton action="start-encounter" onClick={() => run(`Started encounter with ${selected.length} enemy types.`, () => debug.startEncounter(locationId, selected.flatMap(([id, quantity]) => Array.from({ length: quantity }, () => id))))}>START ENCOUNTER</DebugButton></div><p className="debug-note">Draft: {total} / {ENCOUNTER_ENEMY_CAP} enemy instances. Source zones organize the catalogue only; mixed encounters remain allowed.</p></DebugSection><DebugSection title="CURRENT DRAFT" collapsible open={total > 0} onToggle={() => undefined}><div className="debug-draft-list">{selected.length === 0 ? <span className="debug-note">No enemies selected.</span> : selected.map(([id, quantity]) => <div key={id}><span>{labelForEnemy(id)}</span><strong>×{quantity}</strong></div>)}<button type="button" onClick={() => setQuantities({})}>CLEAR</button></div></DebugSection><DebugSection title="WORLD ENCOUNTERS"><div className="debug-catalogue debug-catalogue-tree">{nodes.filter((node) => collectionNodeMatchesSearch(node, query)).map((node) => <EncounterGroup key={node.id} node={node} depth={0} query={query} expanded={expanded} quantities={quantities} onToggle={(id) => setExpanded((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; })} onQuantity={setQuantity} />)}</div></DebugSection></div>;
}
