...
function DebugItemRow({ item, quantity, instanceIds, equippedIds, run, debug, defensiveContext, masteryLevel }: { item: (typeof itemDefinitions)[number]; quantity: number; instanceIds: string[]; equippedIds: Set<string>; run: DebugTabProps["run"]; debug: DebugTabProps["debug"]; defensiveContext: ReturnType<typeof getDefensiveEquipmentContext>; masteryLevel: number }) {
  ...
  return <div className="debug-catalogue-row debug-item-row" data-debug-kind="debug-item" data-debug-item-id={item.id} data-debug-instance-ids={instanceIds.join(",")}><DebugCatalogueIdentity tooltip={buildItemTooltip(item, { quantity, masteryLevel, defensiveContext, equipped })} icon={item.icon} variant={variant} kind="debug-item-identity" targetId={item.id} label={item.name}><strong>{item.name}</strong><small>{item.rarity} - {family}{item.requiredMasteryLevel ? ` - Requires Lv ${item.requiredMasteryLevel}` : ""}</small><em>{quantity} owned{equipped ? " - EQUIPPED" : ""}</em></DebugCatalogueIdentity><button type="button" onClick={() => run(`Granted 1 x ${item.name}.`, () => debug.grantItem(item.id, 1))} data-debug-kind="debug-action" data-debug-action="grant-item" data-debug-item-id={item.id}>+1</button><button type="button" onClick={() => run(`Granted 10 x ${item.name}.`, () => debug.grantItem(item.id, 10))} data-debug-kind="debug-action" data-debug-action="grant-item" data-debug-item-id={item.id}>+10</button><input value={amount} onChange={(event) => setAmount(event.target.value)} aria-label={`Set ${item.name} owned count`} inputMode="numeric" /><button type="button" onClick={() => run(`Set ${item.name} owned count to ${amount}.`, () => debug.setOwnedItemCount(item.id, Number(amount)))} data-debug-kind="debug-action" data-debug-action="set-owned-count" data-debug-item-id={item.id}>SET</button></div>;
}
