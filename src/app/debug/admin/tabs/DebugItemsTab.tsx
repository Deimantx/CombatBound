...
function DebugCopyInspector({ item, instanceId, game, debug, run, masteryLevel, equipped, copyNumber, onDelete }: { item: ItemDefinition; instanceId: string; game: DebugGameState; debug: DebugTabProps["debug"]; run: DebugTabProps["run"]; masteryLevel: number; equipped: boolean; copyNumber: number; onDelete: (instanceId: string) => void }) {
  ...
  return <section className="debug-copy-inspector" data-debug-kind="debug-copy-inspector" data-debug-instance-id={instanceId}>
    <div className="debug-copy-heading"><div><span className="tiny-label">SELECTED COPY</span><strong>{item.name}</strong><small>{equipped ? "Equipped copy" : "Unequipped copy"} · Mastery {masteryLevel}</small></div><span className="debug-copy-sequence">Copy {copyNumber}</span></div>
