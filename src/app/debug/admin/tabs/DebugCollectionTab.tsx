...
export function DebugCollectionTab({ run, debug, onConfirm }: DebugTabProps & { onConfirm: () => void }) {
...
function CollectionNode({ node, depth, query, expanded, onToggle, game }: { node: CollectionGroupNode; depth: number; query: string; expanded: Set<string>; onToggle: (id: string) => void; game: DebugGameState }) {
...
function CollectionEnemyRow({ enemy, sourceLocations, target }: { enemy: (typeof enemyDefinitions)[number]; sourceLocations: string[]; target?: DebugGameState["collection"]["targets"][string] }) {
