...
export function BottomActivityBar() {
  ...
  if (screen === 'combat') return <div className={`activity-bar is-compact ${active ? 'is-combat' : ''}`} data-ui-region="actionStrip" data-debug-kind="persistent-activity" data-debug-label={active ? 'Active combat' : 'Idle activity'}><div className="activity-icon">{active ? <Swords size={15} /> : <PauseCircle size={15} />}</div><div className="activity-copy"><span className="eyebrow">{active ? 'IN COMBAT' : 'IDLE'}</span><strong>{active ? `${location?.name ?? 'Combat Location'} · ${targetName} · HP ${playerHealth}` : 'Choose a Combat Location above.'}</strong></div></div>
  ...
  return <button className={`activity-bar ${active ? 'is-combat' : ''}`} onClick={() => setScreen('combat')} data-ui-region="actionStrip" data-debug-kind="persistent-activity" data-debug-label={active ? 'Active combat' : 'Idle activity'}>
    ...
    <div className="activity-copy"><span className="eyebrow">{active ? 'IN COMBAT' : 'IDLE'}</span><strong>{active ? location?.name ?? 'Combat Location' : 'No active Hunt'}</strong><small>{active ? `${family ?? 'Enemy family'} · ${combat.phase === 'recovery' ? `Recovering · next target in ${combat.recoveryRemaining.toFixed(1)}s` : targetName}` : 'Choose a Combat Location from the Combat screen.'}</small></div>
    {active && <div className="activity-health"><div className="activity-health-heading"><span><span className="heart-mini">♥</span> {playerHealth}</span><span><Crosshair size={12} /> {targetName}</span></div><ProgressBar value={(combat.playerHp / combat.maxPlayerHp) * 100} variant="health" ariaLabel={playerHealthLabel} /></div>}
