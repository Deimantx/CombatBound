...
export function BottomActivityBar() {
  ...
  const playerHealthLabel = `Player health ${Math.floor(combat.playerHp)} of ${Math.floor(combat.maxPlayerHp)}`
  ...
  return <button className={`activity-bar ${active ? 'is-combat' : ''}`} onClick={() => setScreen('combat')} data-ui-region="actionStrip" data-debug-kind="persistent-activity" data-debug-label={active ? 'Active combat' : 'Idle activity'}>
    ...
    {active && <div className="activity-health"><div className="activity-health-heading"><span><span className="heart-mini">♥</span> {Math.floor(combat.playerHp)} / {Math.floor(combat.maxPlayerHp)}</span><span><Crosshair size={12} /> {alive} alive</span></div><ProgressBar value={(combat.playerHp / combat.maxPlayerHp) * 100} variant="health" ariaLabel={playerHealthLabel} /></div>}
