...
export const GAME_SAVE_KEY = "combatbound-idle-save-v7";
...
export function loadGameSave(): GameSaveV7 | null {
  ...
  try {
    ...
    const migratedV6 = v6Raw
      ? migrateV6Save(JSON.parse(v6Raw) as unknown)
      : null;
    const migratedV5 = !migratedV6 && v5Raw
      ? migrateV5Save(JSON.parse(v5Raw) as unknown)
      : null;
    const migratedV4 = !migratedV6 && !migratedV5 && v4Raw
    ...
    const migratedV3 = !migratedV4 && v3Raw
      ...
      : null;
    const migratedV2 =
      !migratedV6 && !migratedV4 && !migratedV3 && currentLegacyRaw
        ? migrateCurrentSave(JSON.parse(currentLegacyRaw) as unknown)
        : null;
    const migratedV1 =
      !migratedV6 && !migratedV4 && !migratedV3 && !migratedV2 && legacyRaw
        ? migrateLegacySave(JSON.parse(legacyRaw) as unknown)
        : null;
    const migrated = migratedV6
      ?? (migratedV5 ? migrateV6Save(migratedV5) : null)
      ?? (migratedV4 ? migrateV6Save(migrateV5Save(migratedV4)) : null)
      ?? (migratedV3 ? migrateV6Save(migrateV5Save(migrateV4Save(migratedV3))) : null)
      ?? (migratedV2 ? migrateV6Save(migrateV5Save(migrateV4Save(migrateV3Save(migratedV2)))) : null)
      ?? (migratedV1 ? migrateV6Save(migrateV5Save(migrateV4Save(migrateV3Save(migratedV1)))) : null);
...
export function saveGame(save: GameSaveV7) {
