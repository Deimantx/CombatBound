...
export function isGameSave(value: unknown): value is GameSaveV2 {
  ...
  if (Object.entries(progression.purchasedPerks).some(([perkId, rank]) => { const perk = perkById[perkId]; if (!perk || typeof rank !== 'number' || !Number.isInteger(rank)) return true; return rank < 0 || rank > perk.maxRank || (perk.branch !== 'Legacy' && !proficiencyById[perk.proficiencyId].perkIds.includes(perkId)) })) return false
