...
function branchEffects(kind: WeaponBranchKind, index: number, icon: string): ProficiencyPerkEffect[] {
  ...
  if (kind === 'precision') return variants([[stat('accuracyRating', 2), stat('additionalBaseCritChance', .01)], [stat('accuracyRating', 3)], [stat('criticalStrikeMultiplier', .05)], [stat('additionalBaseCritChance', .02)], [damage(.08)], [stat('accuracyRating', 8), stat('additionalBaseCritChance', .03)], [stat('criticalStrikeMultiplier', .25)]])
  if (kind === 'critical') return variants([[stat('additionalBaseCritChance', .015)], [stat('criticalStrikeMultiplier', .06)], [stat('accuracyRating', 3), stat('criticalStrikeMultiplier', .03)], [stat('additionalBaseCritChance', .02)], [stat('additionalBaseCritChance', .05)], [stat('criticalStrikeMultiplier', .2)], [conditional(.35, { type: 'targetHpBelow', fraction: .5 })]])
  ...
  if (kind === 'execution') return variants([[stat('accuracyRating', 2)], [conditional(.04, { type: 'targetHpBelow', fraction: .5 })], [stat('additionalBaseCritChance', .02)], [conditional(.06, { type: 'targetHpBelow', fraction: .35 })], [stat('criticalStrikeMultiplier', .15)], [conditional(.15, { type: 'targetHpBelow', fraction: .35 })], [conditional(.35, { type: 'targetHpBelow', fraction: .2 })]])
