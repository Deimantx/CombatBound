...
export type WeaponBranchKind = 'power' | 'bleed' | 'armor' | 'tempo' | 'precision' | 'critical' | 'guard' | 'parry' | 'dodge' | 'cleave' | 'multi' | 'execution' | 'control' | 'opening' | 'penetration'
...
const conditional = (valuePerRank: number, condition: Extract<ProficiencyPerkEffect, { type: 'weaponConditionalDamageModifier' }>['condition']): ProficiencyPerkEffect => ({ type: 'weaponConditionalDamageModifier', operation: 'addPercent', valuePerRank, condition })
const interval = (valuePerRank: number): ProficiencyPerkEffect => ({ type: 'weaponAttackIntervalModifier', valuePerRank })
...
function branchEffects(kind: WeaponBranchKind, index: number, icon: string): ProficiencyPerkEffect[] {
  ...
  if (kind === 'power') return variants([[damage(.03)], [stat('attackPower', 3)], [damage(.04)], [stat('critDamage', .05)], [conditional(.12, { type: 'targetHpAbove', fraction: .5 })], [damage(.12)], [damage(.2)]])
  ...
  if (kind === 'armor' || kind === 'penetration') return variants([[{ type: 'onWeaponHitApplyEffect', effectId: 'effect.armor-broken', chancePerRank: .05 }], [{ type: 'appliedEffectDurationModifier', effectId: 'effect.armor-broken', valuePerRank: .5 }], [{ type: 'weaponArmorPenetrationModifier', mode: 'flat', valuePerRank: 6 }], [conditional(.12, { type: 'targetHasEffect', effectId: 'effect.armor-broken' })], [{ type: 'weaponArmorPenetrationModifier', mode: 'percent', valuePerRank: .05 }], [{ type: 'weaponArmorPenetrationModifier', mode: 'percent', valuePerRank: .2 }], [{ type: 'weaponArmorPenetrationModifier', mode: 'percent', valuePerRank: .35 }, conditional(.15, { type: 'targetHasEffect', effectId: 'effect.armor-broken' })]])
  if (kind === 'tempo') return variants([[stat('staminaRegen', .5)], [interval(-.02)], [{ type: 'weaponOnHitResourceRestore', resource: 'stamina', amountPerRank: .5 }], [{ type: 'techniqueStaminaDrainModifier', valuePerRank: -.05 }], [{ type: 'conditionalStatModifier', stat: 'attackInterval', operation: 'addPercent', valuePerRank: -.06, condition: { type: 'stamina-above', fraction: .7 } }], [stat('staminaRegen', 1), interval(-.06)], [{ type: 'weaponOnHitAdvanceAttack', chancePerRank: .1, fraction: .3 }]])
  if (kind === 'precision') return variants([[stat('accuracy', 2), stat('critChance', .01)], [stat('accuracy', 3)], [stat('critDamage', .05)], [stat('critChance', .02)], [damage(.08)], [stat('accuracy', 8), stat('critChance', .03)], [stat('critDamage', .25)]])
  if (kind === 'critical') return variants([[stat('critChance', .015)], [stat('critDamage', .06)], [stat('accuracy', 3), stat('critDamage', .03)], [stat('critChance', .02)], [stat('critChance', .05)], [stat('critDamage', .2)], [conditional(.35, { type: 'targetHpBelow', fraction: .5 })]])
  if (kind === 'guard' || kind === 'parry') {
    const defenseStat = kind === 'parry' ? stat('parryChance', .015) : stat('blockChance', .015)
    const counter: ProficiencyPerkEffect = kind === 'parry' ? { type: 'onParryApplyEffect', effectId: 'effect.riposte-form', durationSeconds: 3 } : { type: 'onBlockApplyEffect', effectId: 'effect.guarded-counter', durationSeconds: 4 }
    return variants([[defenseStat], [stat('armor', 3)], [kind === 'parry' ? stat('parryChance', .02) : { type: 'weaponBlockPenetrationModifier', stat: 'blockPower', valuePerRank: .1 }], [counter], [kind === 'parry' ? stat('parryChance', .02) : { type: 'weaponBlockPenetrationModifier', stat: 'blockChance', valuePerRank: .05 }], [kind === 'parry' ? { type: 'onParryApplyEffect', effectId: 'effect.counter-window', durationSeconds: 3 } : { type: 'onBlockApplyEffect', effectId: 'effect.counterweight', durationSeconds: 3 }], [kind === 'parry' ? stat('parryChance', .05) : { type: 'weaponBlockPenetrationModifier', stat: 'blockPower', valuePerRank: .2 }, stat('armor', 8)]])
  }
  if (kind === 'dodge') return variants([[stat('dodgeChance', .015)], [stat('evasion', 3)], [{ type: 'onDodgeApplyEffect', effectId: icon === 'bow' ? 'effect.mobile-archer' : 'effect.dagger-evasion', durationSeconds: 3 }], [stat('dodgeChance', .02)], [interval(-.04)], [{ type: 'onDodgeApplyEffect', effectId: icon === 'bow' ? 'effect.mobile-archer' : 'effect.dagger-evasion', durationSeconds: 3 }], [stat('dodgeChance', .04), damage(.1)]])
  ...
  if (kind === 'execution') return variants([[stat('accuracy', 2)], [conditional(.04, { type: 'targetHpBelow', fraction: .5 })], [stat('critChance', .02)], [conditional(.06, { type: 'targetHpBelow', fraction: .35 })], [stat('critDamage', .15)], [conditional(.15, { type: 'targetHpBelow', fraction: .35 })], [conditional(.35, { type: 'targetHpBelow', fraction: .2 })]])
