...
function branchEffects(kind: WeaponBranchKind, index: number, icon: string): ProficiencyPerkEffect[] {
  ...
  if (kind === 'armour' || kind === 'penetration') return variants([[{ type: 'onWeaponHitApplyEffect', effectId: 'effect.armor-broken', chancePerRank: .05 }], [{ type: 'appliedEffectDurationModifier', effectId: 'effect.armor-broken', valuePerRank: .5 }], [{ type: 'weaponArmorPenetrationModifier', mode: 'flat', valuePerRank: 6 }], [conditional(.12, { type: 'targetHasEffect', effectId: 'effect.armor-broken' })], [{ type: 'weaponArmorPenetrationModifier', mode: 'percent', valuePerRank: .05 }], [{ type: 'weaponArmorPenetrationModifier', mode: 'percent', valuePerRank: .2 }], [{ type: 'weaponArmorPenetrationModifier', mode: 'percent', valuePerRank: .35 }, conditional(.15, { type: 'targetHasEffect', effectId: 'effect.armor-broken' })]])
  if (kind === 'tempo') return variants([[stat('staminaRegen', .5)], [interval(-.02)], [{ type: 'weaponOnHitResourceRestore', resource: 'stamina', amountPerRank: .5 }], [{ type: 'techniqueStaminaDrainModifier', valuePerRank: -.05 }], [{ type: 'conditionalStatModifier', stat: 'attackInterval', operation: 'increased', valuePerRank: -.06, condition: { type: 'stamina-above', fraction: .7 } }], [stat('staminaRegen', 1), interval(-.06)], [{ type: 'weaponOnHitAdvanceAttack', chancePerRank: .1, fraction: .3 }]])
  if (kind === 'precision') return variants([[stat('accuracyRating', 2), stat('baseCritChance', .01)], [stat('accuracyRating', 3)], [stat('criticalStrikeMultiplier', .05)], [stat('baseCritChance', .02)], [damage(.08)], [stat('accuracyRating', 8), stat('baseCritChance', .03)], [stat('criticalStrikeMultiplier', .25)]])
  if (kind === 'critical') return variants([[stat('baseCritChance', .015)], [stat('criticalStrikeMultiplier', .06)], [stat('accuracyRating', 3), stat('criticalStrikeMultiplier', .03)], [stat('baseCritChance', .02)], [stat('baseCritChance', .05)], [stat('criticalStrikeMultiplier', .2)], [conditional(.35, { type: 'targetHpBelow', fraction: .5 })]])
  if (kind === 'guard') return variants([[stat('attackBlockChance', .015)], [stat('armour', 3)], [{ type: 'weaponDamageModifier', valuePerRank: .04 }], [{ type: 'onBlockApplyEffect', effectId: 'effect.guarded-counter', durationSeconds: 4 }], [{ type: 'weaponDamageModifier', valuePerRank: .06 }], [{ type: 'onBlockApplyEffect', effectId: 'effect.counterweight', durationSeconds: 3 }], [{ type: 'weaponDamageModifier', valuePerRank: .1 }, stat('armour', 8)]])
  ...
  if (kind === 'execution') return variants([[stat('accuracyRating', 2)], [conditional(.04, { type: 'targetHpBelow', fraction: .5 })], [stat('baseCritChance', .02)], [conditional(.06, { type: 'targetHpBelow', fraction: .35 })], [stat('criticalStrikeMultiplier', .15)], [conditional(.15, { type: 'targetHpBelow', fraction: .35 })], [conditional(.35, { type: 'targetHpBelow', fraction: .2 })]])
