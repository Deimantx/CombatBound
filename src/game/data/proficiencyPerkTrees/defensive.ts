...
const branchData: Record<DefensiveId, { name: string; icon: string; root: string; branches: [BranchSpec, BranchSpec, BranchSpec, BranchSpec, BranchSpec]; apex: string; effects: (branch: number, index: number) => ProficiencyPerkEffect[] }> = {
  ...
  'heavy-armor': {
    ...
    effects: (branch, index) => {
      ...
      if (branch === 1) return [index === 0 ? stat('armour', 3) : index === 1 ? stat('armour', 4, 2) : index === 2 ? stat('attackBlockChance', .02, 2) : index === 3 ? stat('armour', 7, 2) : index === 4 ? stat('attackBlockChance', .01, 3) : index === 5 ? stat('armour', 12, 3) : stat('armour', 24, 4)]
      ...
      if (branch === 3) return [index === 0 ? stat('ailmentDurationReduction', .01) : index === 1 ? stat('ailmentDurationReduction', .015, 2) : index === 2 ? stat('additionalPhysicalDamageReduction', .01, 2) : index === 3 ? stat('ailmentDurationReduction', .02, 2) : index === 4 ? resistance('fire', .01, 3) : index === 5 ? stat('ailmentDurationReduction', .03, 3) : stat('ailmentDurationReduction', .08, 4)]
  ...
  shield: {
    ...
    effects: (branch, index) => {
      if (branch === 0) return [index === 0 ? stat('attackBlockChance', .01) : index === 1 ? stat('attackBlockChance', .015, 1) : index === 2 ? stat('attackBlockChance', .02, 2) : index === 3 ? stat('attackBlockChance', .02, 2) : index === 4 ? stat('attackBlockChance', .04, 3) : index === 5 ? stat('attackBlockChance', .03, 3) : stat('attackBlockChance', .08, 4)]
      if (branch === 1) return [index === 0 ? stat('attackBlockChance', .02) : index === 1 ? stat('armour', 3, 1) : index === 2 ? stat('attackBlockChance', .03, 2) : index === 3 ? stat('armour', 5, 2) : index === 4 ? stat('attackBlockChance', .05, 3) : index === 5 ? stat('ailmentDurationReduction', .02, 3) : stat('attackBlockChance', .12, 4)]
      ...
      return [index === 0 ? resistance('fire', .01) : index === 1 ? resistance('cold', .01, 1) : index === 2 ? stat('armour', 2, 2) : index === 3 ? resistance('lightning', .015, 2) : index === 4 ? resistance('chaos', .02, 3) : index === 5 ? stat('additionalPhysicalDamageReduction', .02, 3) : resistance('cold', .04, 4)]
    },
  },
}
