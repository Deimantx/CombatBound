...
const branchData: Record<DefensiveId, { name: string; icon: string; root: string; branches: [BranchSpec, BranchSpec, BranchSpec, BranchSpec, BranchSpec]; apex: string; effects: (branch: number, index: number) => ProficiencyPerkEffect[] }> = {
  'light-armor': {
    ...
    effects: (branch, index) => {
      if (branch === 0) return [index === 0 ? stat('manaRegen', .15) : index === 1 ? stat('evasion', 1, 2) : index === 2 ? spell('accuracy', 2, 2) : index === 3 ? spell('cooldown', -.03, 2) : index === 4 ? stat('dodgeChance', .01, 3) : index === 5 ? spell('manaCost', -.05, 3) : stat('manaRegen', .5, 4)]
      if (branch === 1) return [index === 0 ? stat('maxMana', 4) : index === 1 ? stat('maxMana', 8, 2) : index === 2 ? spell('manaCost', -.03, 2) : index === 3 ? stat('manaRegen', .2, 2) : index === 4 ? spell('barrierAmount', .05, 3) : index === 5 ? stat('maxMana', 12, 3) : spell('manaCost', -.1, 4)]
      if (branch === 2) return [index === 0 ? stat('evasion', 2) : index === 1 ? stat('dodgeChance', .01, 2) : index === 2 ? stat('evasion', 3, 2) : index === 3 ? stat('dodgeChance', .015, 2) : index === 4 ? stat('parryChance', .01, 3) : index === 5 ? stat('evasion', 8, 3) : stat('dodgeChance', .04, 4)]
      if (branch === 3) return [index === 0 ? spell('barrierAmount', .04, 1) : index === 1 ? spell('barrierAmount', .06, 2) : index === 2 ? stat('healthRegen', .1, 2) : index === 3 ? spell('barrierDuration', .1, 2) : index === 4 ? resistance('fire', .01, 3) : index === 5 ? stat('maxHealth', 10, 3) : spell('barrierAmount', .15, 4)]
      return [index === 0 ? resistance('fire', .01) : index === 1 ? resistance('water', .01, 2) : index === 2 ? resistance('air', .01, 2) : index === 3 ? resistance('earth', .01, 2) : index === 4 ? resistance('light', .015, 3) : index === 5 ? resistance('darkness', .015, 3) : resistance('mystic', .03, 4)]
    },
  },
  'medium-armor': {
    ...
    effects: (branch, index) => {
      ...
      if (branch === 1) return [index === 0 ? stat('evasion', 2) : index === 1 ? stat('dodgeChance', .01, 2) : index === 2 ? stat('evasion', 3, 2) : index === 3 ? stat('dodgeChance', .015, 2) : index === 4 ? stat('evasion', 6, 3) : index === 5 ? stat('dodgeChance', .025, 3) : stat('evasion', 12, 4)]
      if (branch === 2) return [index === 0 ? weapon('accuracy', 2, 1) : index === 1 ? weapon('damage', .03, 2) : index === 2 ? weapon('attackInterval', -.02, 2) : index === 3 ? stat('accuracy', 3, 2) : index === 4 ? weapon('damage', .06, 3) : index === 5 ? weapon('attackInterval', -.04, 3) : weapon('damage', .12, 4)]
      if (branch === 3) return [index === 0 ? stat('armor', 2) : index === 1 ? stat('blockChance', .01, 2) : index === 2 ? stat('blockPower', .02, 2) : index === 3 ? stat('armor', 4, 2) : index === 4 ? stat('blockChance', .015, 3) : index === 5 ? stat('blockPower', .04, 3) : stat('armor', 12, 4)]
      return [index === 0 ? stat('maxHealth', 5) : index === 1 ? stat('statusResistance', .01, 2) : index === 2 ? resistance('physical', .01, 2) : index === 3 ? stat('maxHealth', 8, 2) : index === 4 ? resistance('fire', .01, 3) : index === 5 ? stat('statusResistance', .02, 3) : resistance('physical', .04, 4)]
    },
  },
  'heavy-armor': {
    ...
    branches: [
      ...
      { name: 'Last Stand', icon: 'flame', names: ['Pressure Tested', 'Low Health Guard', 'Pain into Power', 'Desperate Strength', 'Final Bulwark', 'Deathless Stance', 'Last Stand'] },
    ],
    effects: (branch, index) => {
      if (branch === 0) return [index === 0 ? stat('maxHealth', 10) : index === 1 ? stat('maxHealth', 15, 2) : index === 2 ? stat('maxHealth', 20, 2) : index === 3 ? stat('healthRegen', .15, 2) : index === 4 ? stat('maxHealth', 35, 3) : index === 5 ? stat('healthRegen', .3, 3) : stat('maxHealth', 60, 4)]
      if (branch === 1) return [index === 0 ? stat('armor', 3) : index === 1 ? stat('armor', 4, 2) : index === 2 ? stat('blockPower', .02, 2) : index === 3 ? stat('armor', 7, 2) : index === 4 ? stat('blockChance', .01, 3) : index === 5 ? stat('armor', 12, 3) : stat('armor', 24, 4)]
      if (branch === 2) return [index === 0 ? stat('healthRegen', .15) : index === 1 ? stat('healthRegen', .2, 2) : index === 2 ? stat('healthRegen', .25, 2) : index === 3 ? stat('maxHealth', 10, 2) : index === 4 ? stat('healthRegen', .35, 3) : index === 5 ? stat('healthRegen', .5, 3) : stat('healthRegen', 1, 4)]
      if (branch === 3) return [index === 0 ? stat('statusResistance', .01) : index === 1 ? stat('statusResistance', .015, 2) : index === 2 ? resistance('physical', .01, 2) : index === 3 ? stat('statusResistance', .02, 2) : index === 4 ? resistance('fire', .01, 3) : index === 5 ? stat('statusResistance', .03, 3) : stat('statusResistance', .08, 4)]
      return [index === 0 ? stat('armor', 2) : index === 1 ? stat('maxHealth', 10, 2) : index === 2 ? stat('armor', 4, 2) : index === 3 ? stat('healthRegen', .2, 2) : index === 4 ? stat('armor', 8, 3) : index === 5 ? stat('maxHealth', 20, 3) : stat('healthRegen', .75, 4)]
    },
  },
  shield: {
    ...
    branches: [
      ...
      { name: 'Block Power', icon: 'shield', names: ['Braced Impact', 'Dense Guard', 'Absorbing Face', 'Reinforced Rim', 'Heavy Deflection', 'Warding Block', 'Unstoppable Guard'] },
    ...
    effects: (branch, index) => {
      if (branch === 0) return [index === 0 ? stat('blockChance', .01) : index === 1 ? stat('blockChance', .015, 1) : index === 2 ? stat('blockPower', .02, 2) : index === 3 ? stat('blockChance', .02, 2) : index === 4 ? stat('blockPower', .04, 3) : index === 5 ? stat('blockChance', .03, 3) : stat('blockChance', .08, 4)]
      if (branch === 1) return [index === 0 ? stat('blockPower', .02) : index === 1 ? stat('armor', 3, 1) : index === 2 ? stat('blockPower', .03, 2) : index === 3 ? stat('armor', 5, 2) : index === 4 ? stat('blockPower', .05, 3) : index === 5 ? stat('statusResistance', .02, 3) : stat('blockPower', .12, 4)]
      ...
      return [index === 0 ? resistance('fire', .01) : index === 1 ? resistance('water', .01, 1) : index === 2 ? stat('statusResistance', .02, 2) : index === 3 ? resistance('air', .015, 2) : index === 4 ? resistance('darkness', .02, 3) : index === 5 ? resistance('physical', .02, 3) : resistance('mystic', .04, 4)]
    },
  },
}
