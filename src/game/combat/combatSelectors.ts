...
import { applyProficiencyStatModifiers, getConditionalMagicStatModifiers, getConditionalProficiencyStatModifiers } from '../progression/perkProgression'
...
import type { CombatState, CombatStats, EnemyCombatInstance } from './combatTypes'
...
export function getPlayerEffectiveCombatStats(combat: CombatState, stats: HunterCombatStats, progression?: ProgressionState, effectDefinitions = effectById): CombatStats {
  const base = calculateEffectiveCombatStats(normalizeCombatStats(stats as HunterCombatStats & Record<string, unknown>), combat.playerEffects, effectDefinitions)
  if (!progression) return base
  const barrierActive = getBarrierAmount(combat.playerEffects, effectDefinitions) > 0
...
export function getEnemyEffectiveCombatStats(enemy: EnemyCombatInstance, effectDefinitions = effectById): CombatStats {
  const definition = enemyById[enemy.enemyId]
  return calculateEffectiveCombatStats(calculateEnemyBaseCombatStats(definition), enemy.effects, effectDefinitions)
}

export function getPlayerBarrierAmount(combat: CombatState, effectDefinitions = effectById) {
  return getBarrierAmount(combat.playerEffects, effectDefinitions)
}

export function getSelectedTargetMatchup(combat: CombatState, stats: HunterCombatStats, progression: ProgressionState, selectedEnemy?: EnemyCombatInstance): CombatMatchupView | null {
  ...
  return {
    ...
    playerHitChance: calculateHitChance(playerStats.accuracy, enemyStats.evasion),
    targetDodgeChance: enemyStats.dodgeChance,
    targetParryChance: enemyStats.parryChance,
    targetBlockChance: enemyStats.blockChance,
    enemyAccuracy: enemyStats.accuracy,
    playerEvasion: playerStats.evasion,
    enemyHitChance: calculateHitChance(enemyStats.accuracy, playerStats.evasion),
...
export function getHuntSessionRates(session: CombatState['session']): HuntSessionRates {
  const seconds = Math.max(0, Number.isFinite(session.elapsedSeconds) ? session.elapsedSeconds : 0)
  const safeAmount = (value: number | undefined) => value !== undefined && Number.isFinite(value) ? Math.max(0, value) : 0
  const perSecond = (value: number) => seconds > 0 ? safeAmount(value) / seconds : 0
  const perHour = (value: number) => seconds >= MIN_RATE_SAMPLE_SECONDS ? perSecond(value) * 3600 : 0
  const kills = safeAmount(session.enemiesDefeated)
  const totalProficiencyXp = Object.values(session.proficiencyXpGained).reduce((sum, value) => sum + safeAmount(value), 0)
  const proficiencyXpPerHourById = Object.fromEntries(Object.entries(session.proficiencyXpGained).map(([id, value]) => [id, perHour(value ?? 0)])) as Partial<Record<CombatProficiencyId, number>>
  return {
