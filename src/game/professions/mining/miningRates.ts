...
export function estimateMiningRates(game: MiningRuntimeGame): MiningRateEstimate {
  ...
  const initialRemainder = initial.mining.yieldRemainders["item.iron-ore"] ?? 0
  const finalRemainder = result.game.mining.yieldRemainders["item.iron-ore"] ?? 0
  const oreProduction = Math.max(0, result.summary.ironOre + finalRemainder - initialRemainder)
  return { miningXpPerHour: result.summary.miningXp * 3600 / seconds, masteryXpPerHour: result.summary.masteryXp * 3600 / seconds, ironOrePerHour: oreProduction * 3600 / seconds, roughGemPerHour: result.summary.expectedRoughGems * 3600 / seconds, blackStonePerHour: result.summary.expectedBlackStones * 3600 / seconds, activeSwingFraction: Math.max(0, Math.min(1, (seconds - result.summary.restSeconds) / seconds)), restFraction: Math.max(0, Math.min(1, result.summary.restSeconds / seconds)), secondsPerProjection }
}
