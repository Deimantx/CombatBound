...
describe('combat information surfaces', () => {
  it('renders the complete collapsible Hero Combat Stats inspector', () => {
    ...
    for (const label of ['Attack Damage', 'Accuracy Rating', 'Attack Interval', 'Base Critical Chance', 'Critical Strike Multiplier', 'Max Life', 'Armour', 'Evasion Rating', 'Attack Block Chance', 'Spell Block Chance', 'Spell Suppression Chance', 'Max Stamina', 'Stamina Regeneration', 'Max Mana', 'Mana Regeneration', 'Fire Resistance', 'Cold Resistance', 'Lightning Resistance', 'Chaos Resistance']) expect(screen.getAllByText(label).length).toBeGreaterThan(0)
    ...
    expect(defenseLabels[defenseLabels.indexOf('Evasion Rating') - 1]).toBe('Additional Physical Damage Reduction')
    ...
    expect(screen.getByText('Base Critical Chance')).not.toBeVisible()
