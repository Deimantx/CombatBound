...
function harness(adapter: OfflineActivityAdapter<FakeState, FakeSummary>, overrides: Partial<Parameters<typeof runOfflineActivityTransaction<FakeState, FakeSummary>>[0]> = {}) {
  ...
  const result = (requestedSeconds: number, activitySeconds = requestedSeconds): OfflineActivitySimulationResult<FakeState, FakeSummary> => ({
    requestedSeconds,
    activitySeconds,
    bankSpentSeconds: requestedSeconds,
    wastedSeconds: requestedSeconds - activitySeconds,
    stopReason: activitySeconds === requestedSeconds ? "requested-time-complete" : "activity-ended",
    state: { active: true, value: activitySeconds },
    summary: { note: "fake" },
  });
  const options = {
...
describe("Offline Activity Simulation Contract 1.0", () => {
  ...
  it("commits the full requested skip even when activity ends early", () => {
    const exact = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: request.requestedSeconds, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 0, stopReason: "requested-time-complete", state: { active: true, value: request.requestedSeconds }, summary: { note: "exact" } })));
    expect(exact.run()).toMatchObject({ ok: true, activityType: "fake-activity" });
    expect(exact.getCommitted()?.result.activitySeconds).toBe(900);

    const partial = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: 600, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 300, stopReason: "death", state: { active: false, value: 600 }, summary: { note: "stopped" } })));
    const result = partial.run();
    expect(result).toMatchObject({ ok: true });
    expect(partial.getCommitted()?.result.activitySeconds).toBe(600);
    expect(partial.getCommitted()?.result.bankSpentSeconds).toBe(900);
    expect(partial.getCommitted()?.result.wastedSeconds).toBe(300);
    expect(partial.getCommitted()?.requestedSeconds).toBe(900);
  });

  it("rejects a successful result that claims zero bank spend", () => {
    const zero = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: 0, bankSpentSeconds: 0, wastedSeconds: 0, stopReason: "activity-ended", state: { active: false, value: 0 }, summary: { note: "empty" } })));
    expect(zero.run()).toMatchObject({ ok: false, error: "invalid-result" });
    expect(zero.getCommitted()).toBeNull();
  });
  ...
  it("does not commit thrown or invalid simulations", () => {
    ...
    const invalid = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: request.requestedSeconds + 1, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 0, stopReason: "requested-time-complete", state: { active: true, value: 1 }, summary: { note: "bad" } })));
    expect(invalid.run()).toMatchObject({ ok: false, error: "invalid-result" });
    expect(invalid.getCommitted()).toBeNull();
    const commitThrows = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: request.requestedSeconds, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 0, stopReason: "requested-time-complete", state: { active: true, value: 1 }, summary: { note: "fake" } })), {
  ...
  it("checks lease ownership before and after simulation", () => {
    const before = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: request.requestedSeconds, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 0, stopReason: "requested-time-complete", state: { active: true, value: 1 }, summary: { note: "fake" } })));
    ...
    after = harness(makeAdapter((_state, request) => {
      ...
      return { ...request, activitySeconds: request.requestedSeconds, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 0, stopReason: "requested-time-complete", state: { active: true, value: 1 }, summary: { note: "fake" } };
    }));
  ...
  it("prevents concurrent requests and uses the seven-day policy boundary", () => {
    const running = harness(makeAdapter((_state, request) => ({ ...request, activitySeconds: request.requestedSeconds, bankSpentSeconds: request.requestedSeconds, wastedSeconds: 0, stopReason: "requested-time-complete", state: { active: true, value: 1 }, summary: { note: "fake" } })));
