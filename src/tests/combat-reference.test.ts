...
describe('combat reference metadata', () => {
  it('keeps the important stat distinctions explicit', () => {
    expect(combatStatReferenceById.accuracyRating.fullDescription).toContain('Evasion')
    expect(combatStatReferenceById.accuracyRating.fullDescription).not.toContain('Armour affects Hit Chance')
  ...
  it('uses compact combat formats and resistance tones', () => {
    expect(formatCombatStatValue('criticalStrikeMultiplier', 1.5)).toBe('150%')
    expect(formatCombatStatValue('attackInterval', 2.4)).toBe('2.4s')
    expect(formatCombatStatValue('staminaRegen', 5)).toBe('5.0 / sec')
    expect(formatCombatStatValue('manaRegenFlat', 1)).toBe('1.0 / sec')
    expect(formatItemStat('baseAttackTime', 2.4).value).toBe('2.4s')
